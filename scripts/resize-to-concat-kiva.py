...
def resize_and_split_images(
    ...
    canvas, _ = renderer.draw_padded_kiva_problem(
        images,
    ...
    for idx, col_name in enumerate(image_cols[: len(images)]):
...
def main():
    parser = argparse.ArgumentParser(
        description="Resize concatenated Kiva grid and split back into individual image columns."
    )
    parser.add_argument(
        "--dataset",
        required=True,
