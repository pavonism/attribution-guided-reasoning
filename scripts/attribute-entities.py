...
Use the format: <answer>["entity1 caption", "entity2 caption", ...]</answer>.
""".strip()
...
class ConversationContext:
        ...
        return [
            {
                "role": "user",
                "content": [
                    make_text_content(self.question),
                    make_text_content("Positive Image:"),
                    make_image_content(self.positive_image, ConversationFormat.VLLM),
                    make_text_content("Negative Image:"),
                    make_image_content(self.negative_image, ConversationFormat.VLLM),
                ],
            },
        ]
    ...
    def make_final_answer_conversation(self):
        return [
            ...
            {
                "role": "user",
                "content": [make_text_content(FINAL_ANSWER_PROMPT)],
            },
        ]
...
def attribute_entities(
    ...
    responses = model.chat(
        messages=[c.make_conversation() for c in conversation_contexts],
        sampling_params=sampling_params,
    )

    for response, conversation_context in zip(responses, conversation_contexts):
        conversation_context.reasoning = response.outputs[0].text

    responses = model.chat(
        messages=[c.make_final_answer_conversation() for c in conversation_contexts],
        sampling_params=sampling_params,
    )
    ...
    for response, conversation_context in zip(responses, conversation_contexts):
        ...
        conversation_context.entities.update(parse_entities(response.outputs[0].text))
...
def main():
    ...
    model = LLM(
        ...
        max_model_len=32768 + max_tokens,
        enforce_eager=True,
        dtype="bfloat16",
        seed=seed,
